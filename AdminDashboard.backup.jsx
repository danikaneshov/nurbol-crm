...
const AdminDashboard = () => {
  ...
  const [operationType, setOperationType] = useState('in'); // 'in' | 'writeoff' для объединённых операций склада
  ...
  // Финансовые данные для дашборда (используют dashboardMonth)
  const dashboardTamerlanEarned = closedSystemShifts
    .filter(s => s.employeeName && s.employeeName.trim().toLowerCase() === 'tamerlan')
    .reduce((a, b) => a + (b.earned || 0), 0);
  const dashboardPurchases = invMovements
    .filter(m => m.type === 'in' && (dashboardMonth === 'all' || (m.dateStr && m.dateStr.endsWith(`.${dashboardMonth}`))))
    .reduce((a, b) => a + (b.cost || 0), 0);
  const dashboardNetProfit = globalOwnerProfit - totalSystemEarned;
  const dashboardProfitWithoutTamerlan = globalOwnerProfit - (totalSystemEarned - dashboardTamerlanEarned);
  const dashboardHookahProfit = globalHookahs * ownerProfits.hookah;
  const dashboardReplacementProfit = globalReplacements * ownerProfits.replacement;
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-gradient-to-br from-green-500 to-green-700 p-8 rounded-[32px] shadow-lg shadow-green-200 text-white relative overflow-hidden md:col-span-2">
                ...
                <div className="flex flex-col sm:flex-row gap-8 justify-between relative z-10">
                  <div>
                    <p className="font-bold text-sm uppercase tracking-widest mb-2 opacity-80">Чистая прибыль</p>
                    <h3 className="text-4xl font-black">{formatMoney(dashboardNetProfit)} ₸</h3>
                    <p className="text-sm opacity-80 mt-2">Вычеты: ЗП ({formatMoney(totalSystemEarned)} ₸)</p>
                  </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[600px]">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {dashboardProfitByMaster.map(emp => (
