...
import { Trash2, Edit3, Leaf, Plus } from 'lucide-react';
...
const TobaccoTypesTab = ({ tobaccoTypes }) => {
  const [form, setForm] = useState({ name: '', pricePerGram: '' });
  const [isAdding, setIsAdding] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [editForm, setEditForm] = useState({ name: '', pricePerGram: '' });

  const handleAdd = async (e) => {
    e.preventDefault();
    if (!form.name || !form.pricePerGram) return;
    setIsAdding(true);
    try {
  ...
  const handleSaveEdit = async (id) => {
    try {
      await updateDoc(doc(db, 'tobacco_types', id), {
        ...
        pricePerGram: Number(editForm.pricePerGram)
      });
      setEditingId(null);
    } catch (err) { alert('Ошибка: ' + err.message); }
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 animate-in fade-in duration-300">
      ...
      <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm h-fit">
        ...
        <form onSubmit={handleAdd} className="space-y-4">
          ...
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1 ml-1">Цена за 1 грамм (₸)</label>
            <input
              type="number"
              min="0"
              step="0.01"
          ...
          <button
            type="submit"
            disabled={isAdding || !form.name || !form.pricePerGram}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {tobaccoTypes.map(tt => (
              <Card key={tt.id} className="p-6 bg-white border border-slate-100 hover:border-green-200 hover:shadow-lg hover:shadow-green-50 hover:-translate-y-1 transition-all">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 font-medium mt-0.5">{formatMoney(tt.pricePerGram)} ₸/г</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    {editingId !== tt.id && (
                      <button
                        onClick={() => { setEditingId(tt.id); setEditForm({ name: tt.name, pricePerGram: tt.pricePerGram }); }}
                ...
                {/* Быстрая метрика: цена за 50г, 100г, 250г */}
                {editingId !== tt.id ? (
                  <div className="grid grid-cols-3 gap-2 mt-3 bg-slate-50 p-3 rounded-xl">
                    {[50, 100, 250].map(g => (
                      <div key={g} className="text-center">
                        <p className="text-[9px] font-bold text-slate-400 uppercase">{g}г</p>
                        <p className="font-black text-slate-700 text-sm">{formatMoney(Math.round(tt.pricePerGram * g))} ₸</p>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-100 mt-3 animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Цена за 1г (₸)</label>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
